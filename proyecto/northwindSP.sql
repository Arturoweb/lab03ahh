...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 02-06-2019 a las 00:21:55
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizarProducto` (IN `codP` INT, `nomP` VARCHAR(50), `catP` INT, `provP` INT, `prcP` DOUBLE, `stckP` INT)  UPDATE products
SET ProductName = nomP, CategoryID = catP, SupplierID = provP,
	UnitPrice = prcP, UnitsInStock = stckP
WHERE ProductID = codP$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `buscarProducto` (IN `codP` INT)  SELECT ProductID, ProductName, UnitPrice,
    	   UnitsInStock, CategoryID, SupplierID
    FROM products
    WHERE ProductID=codP$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `busquedaProducto` (IN `cod` INT)  SELECT* 
    FROM products
    WHERE ProductID=cod$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `consultaProducto` ()  SELECT p.ProductID, p.ProductName, p.UnitPrice, p.UnitsInStock, c.CategoryName,
    		s.CompanyName 
    FROM products p
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `crearProducto` (IN `nomP` VARCHAR(40), `catP` INT, `SupP` INT, `UntP` DOUBLE, `uStock` INT)  INSERT INTO products (ProductName, 
                      CategoryID, 
                      SupplierID, 
                      UnitPrice, 
                      UnitsInStock)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listaCliente` ()  SELECT CustomerId, CompanyName
FROM customers
ORDER BY CustomerId$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listaEmpleado` ()  SELECT EmployeeID, ( CONCAT(LastName," ",FirstName)) AS Nombre 
FROM employees$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listaProducto` ()  SELECT p.ProductID, p.ProductName, p.UnitPrice,p.UnitsInStock,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `lstCategoria` ()  SELECT CategoryId, CategoryName
    FROM categories
    ORDER BY CategoryId ASC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `lstProveedor` ()  SELECT SupplierID, CompanyName
    FROM suppliers
...
CREATE TABLE `customercustomerdemo` (
  `CustomerID` char(5) CHARACTER SET utf8 NOT NULL,
  `CustomerTypeID` char(10) CHARACTER SET utf8 NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `customerdemographics` (
  `CustomerTypeID` char(10) CHARACTER SET utf8 NOT NULL,
  `CustomerDesc` longtext CHARACTER SET utf8
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `employees` (
  ...
  `PhotoPath` varchar(255) CHARACTER SET utf8 DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `employeeterritories` (
  `EmployeeID` int(11) NOT NULL,
  `TerritoryID` varchar(20) CHARACTER SET utf8 NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `orders` (
  `OrderID` int(11) NOT NULL,
  `CustomerID` char(5) CHARACTER SET utf8 DEFAULT NULL,
...
(1, 'Mandarina', 1, 1, '10 boxes x 20 bags', 18, 39, 0, 10, 0),
...
(75, 'Hola', 12, 1, '24 - 0.5 l bottles', 7.75, 125, 0, 25, 0),
(76, 'Lakkalik��ri', 23, 1, '500 ml', 18, 57, 0, 20, 0),
(77, 'Original Frankfurter gr�ne So�e', 12, 2, '12 boxes', 13, 32, 0, 15, 0),
(78, 'Aceite de cocina', 1, 1, NULL, 10.6, 100, 0, 0, 0),
(79, 'Papa Tomasa saco por 10 kilos', 19, 7, NULL, 20, 200, 0, 0, 0),
(80, 'Flores', 1, 6, NULL, 1, 1, 0, 0, 0);
...
CREATE TABLE `region` (
  ...
  `RegionDescription` char(50) CHARACTER SET utf8 NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `suppliers` (
  ...
  `Phone` varchar(24) CHARACTER SET utf8 DEFAULT NULL,
  `Fax` varchar(24) CHARACTER SET utf8 DEFAULT NULL,
  `HomePage` longtext CHARACTER SET utf8
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `territories` (
  `TerritoryID` varchar(20) CHARACTER SET utf8 NOT NULL,
  `TerritoryDescription` char(50) CHARACTER SET utf8 NOT NULL,
  `RegionID` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
